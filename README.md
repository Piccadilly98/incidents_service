...
WEBHOOK_URL=http://localhost:8080/test  # url для отправки вебхука: дефолтное значение: http://localhost:8080/test    
...
![Успешная регистрация](screenshots/success_registration.png)

### Ошибка регистрации: неверный API-ключ
![Ошибка регистрации — invalid API-key](screenshots/err_registration_api_key.png)

### Ошибка регистрации: невалидная широта/долгота
![Ошибка регистрации — invalid latitude/longitude](screenshots/err_registration_invalid_latitude_long.png)

### Успешное получение инцидента по ID
![Получение инцидента по ID — 200 OK](screenshots/get_id_succes.png)

### Ошибка получения: инцидент не найден
![Get incident — 404 Not Found](screenshots/get_not_found.png)

### Ошибка получения: невалидный UUID
![Get incident — 400 invalid UUID](screenshots/get_not_uuid.png)

### Health-check: всё работает
![Health-check — 200 OK](screenshots/health_check_ok.png)

### Health-check: ошибка подключения
![Health-check — 503 Service Unavailable](screenshots/health_check_err_ping.png)

### Пагинация списка инцидентов
![Список инцидентов с пагинацией](screenshots/get_pagination_page.png)

### Проверка координат: без обнаруженных зон
![Location check — is_danger: false](screenshots/location_check_not_detected.png)

### Проверка координат: обнаружены зоны
![Location check — is_danger: true](screenshots/location_check_detected.png)
...
**Мой пример:**
![Пример ngrok](screenshots/ngrok_result.png)
На данном скрине:
